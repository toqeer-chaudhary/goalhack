...
// function to validate kpi end date
    ...
    $(document).on('click', '.userEditButton', function () {
        ...
        var userId = $(this).closest("tr")["0"].children["0"].innerHTML;
        var levelName = $(this).closest("tr")["0"].children["4"].innerHTML;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#UserlevelDropDown-edit option:selected").text(levelName);
                $("#CountySel option:selected").text(data.country);
                $("#StateSel option:selected").text(data.province);
                $("#CitySel option:selected").text(data.city);
            ...
            success: function (response) {
                $("#user" + response.id).replaceWith("<tr id='user" + response.id + "'>" +
                   ...
                   /* "<td><button class='btn btn-outline-primary ks-no-text ks-solid userEditButton' id='editButton' data-toggle='modal' data-target='#EditUserModal' title='Edit' data-id='" + response.id + "' id='packageUpdateButton'> " + "<span class=\"la la-edit ks-icon\"></span>" + "</button>" +"&nbsp;"+
                    ...
                    "<div class='dropdown-menu dropdown-menu-right' x-placement='bottom-end' " +
                    "style='position: absolute; transform: translate3d(64px,38px,0px); top: 0px; left: 0px; will-change: transfrom;'>" +
                    "<a class='dropdown-item userEditButton' href='#' data-id='"+response.id+"' data-toggle='modal' data-target='#EditUserModal'>" +
                    "<span class='la la-pencil ks-icon'></span>Edit</a>" +
                    "<a class='dropdown-item deleteuserButton' href='#' data-id='"+response.id+"'>" +
                    "<span class='la la-trash ks-icon'></span>Delete</a>" +
                    // "<a class='dropdown-item assignKpiLink' href='#' data-id='"+response.id+"' data-toggle='modal' data-target='#assignKpiModal'>" +
                    // "<span class='la la-users ks-icon'></span>Assign</a>" +
                    "<a class='dropdown-item user-detail-button' href='#' data-id='"+response.id+"' data-toggle='modal' data-target='#user-detail-modal'>" +
                    "<span class='la la-bar-chart-o ks-icon'></span>Details</a>" +
                    "</div>" +
                    "</div></td>" +
                   "</tr>"
            ...
            if(willDelete) {
                $.ajax({
                    ...
                    success: function (data) {
                        if(data == 1)
                        {
                            swal("subscribed!", "Can't delete this package!", "error");
                        }
                       else{
...
*/
    ...
    $("#create-goal-form").submit(function (e) {
        ...
        if (validateSelectKPI() || validateSelectDataEntryMode() || validateGoalName() || validateGoalTarget() || validateGoalDescription() || validateGoalStartDate() || validateGoalEndDate() )
        {
            ...
            var goalName             = $("#goal-name").val();
            var kpiID   = $("#kpi-dropDown :selected").val();
            $.ajax({
                url: "goalExist/" + goalName + "/" + kpiID,
                type: "get",
                success: function (response) {
                    // 1 means that level name exist
                    if (response == 1) {
                        $("#goal-name-has-error").html("This name has already been taken").css({
                            "color": "red",
                            "font-size": "12px"
...
/*++++++++++++++++++++++++++++ Strategy Date++++++++++++++++++++++++++++*/
    ...
    $(document).on('click', '.user-detail-button', function ()
    {
        var userId = $(this).closest("tr")["0"].children["0"].innerHTML;
        var levelName = $(this).closest("tr")["0"].children["4"].innerHTML;
        //console.log(id);
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#user-detail-name").val(data.name);
                $("#user-detail-last-name").val(data.last_name);
                $("#user-detail-email").val(data.email);
                $("#user-detail-designation").val(data.designation);
                $("#user-detail-contact").val(data.contact);
                $("#user-detail-country").val(data.country);
                $("#user-detail-city").val(data.city);
                $("#user-detail-province").val(data.province);
                $("#user-detail-address").val(data.address);
                $("#user-detail-level").val(levelName);
                //console.log(data.level_id.name);
                    ...
                    for (var i = 0; i <numberOfWeeks; i++)
                    {
                        ...
                        tableData    += '<td class=\"actual past\">'+
                                        ...
                                        '<input type=\"tell\" tabindex="20" autocomplete="off" maxlength="15" class="actual actualdata" id="goal-data'+i+'" value='+actual_goaldata+'>'+
                                         '<input type="text" class="comments" id="goal-data-comment'+i+'" value="'+comments+'">'+
                                        '<input type="hidden" name="dataid" class="goaldataid" value ='+i+' >' +
                                        '</td>';
...
function showVisionCharts()
{
    $('.show-vision-chart').show();
...
$(document).on("click", ".vision-dashboard", function (e) {
    ...
    $.ajax({
        ...
        success:function (response) {
            ...
            var vision_left = 0;
            var vision_target = 0;
            $('.d-vision-name').html(response[6].name);
            ...
            function visionLineDrawChart() {
                ...
                data.addColumn('string', 'Strategy Name');
                data.addColumn('number', 'Strategy Actual');
                data.addColumn('number', 'Strategy Target');

                for (var d_strategy_length = 0; d_strategy_length < d_strategy.length; d_strategy_length++)
                {
                    data.addRows([
                        [d_strategy[d_strategy_length].name , response[8][d_strategy_length], response[7][d_strategy_length]],
                    ]);
                }
                ...
                var chart = new google.visualization.LineChart(document.getElementById('d_vision_curve_chart'));
            ...
            function visionPieDrawChart() {
                ...
                var data = new google.visualization.DataTable();
                data.addColumn('string', 'Strategy Name');
                data.addColumn('number', 'Strategy Achieved');

                for (var d_strategy_length = 0; d_strategy_length <= d_strategy.length; d_strategy_length++)
                {
                    var vi_per = 0;
                    vi_per = parseInt((response[8][d_strategy_length]/response[7][d_strategy_length]) * 100);
                    if (!isNaN(vi_per)) {
                        /*for (var v = 0; v < 2; v++) {
                            ...
                            if (v == 0)
                                ...
                                data.setCell(d_strategy_length, v, vi_per);

                        }*/
                        data.addRows([
                            [d_strategy[d_strategy_length].name ,vi_per],
                        ]);
                    }
                    else
                    {
                ...
                var chartPie = new google.visualization.PieChart(document.getElementById('d_vision_pie_chart'));
            ...
            function visionBarDrawChart() {
                ...
                var barChart = new google.visualization.ColumnChart(document.getElementById('d_vision_bar_chart'));
...
$(document).on("click", ".strategy-dashboard", function (e) {
    ...
    $.ajax({
        ...
        success:function (response) {
            ...
            $('.d-strategy-name').html(response[5].name);
            ...
            function strategyLineDrawChart() {
                ...
                var str_act = 0;
                var str_pers = 0;
                for (var d_kpis_length = 0; d_kpis_length < response[6].length; d_kpis_length++)
                {
                    str_tot += response[6][d_kpis_length];
                    str_act += response[7][d_kpis_length];
                }
                str_pers = parseInt((str_act/str_tot) * 100);
                console.log(str_pers);
                data.addRows([
                    [response[5].name , str_pers],
                ]);*/
            ...
            function strategyPieDrawChart() {
                ...
                {
                    var st_per = 0;
                    st_per = parseInt((response[7][d_kpis_length]/response[6][d_kpis_length]) * 100);
                    if (!isNaN(st_per))
                    {
                        data.addRows([
                            [d_kpis[d_kpis_length].name ,st_per],
                        ]);
                    }
                    else
                    {
    ...
    $.ajax({
        ...
        data:
            ...
            success:function (response) {
                ...
                $('.d-kpi-name').html(response[3].name);
                ...
                function kpiPieDrawChart() {
                    ...
                    for (d_goal_length = 0; (d_goal_length < d_goals_name.length) || (response[0].length == 0) ; d_goal_length++)
                    {
                ...
                function kpiBarDrawChart() {
                    ...
                    for (d_goal_length = 0; d_goal_length < d_goals_name.length; d_goal_length++)
                    {
                        ...
                        {
                            data.addRows([
                                [d_goals_name[d_goal_length].name ,response[1][d_goal_length], 1],
                            ]);
                        }
...
$(document).on("click", ".goal-dashboard", function (e) {
    ...
    $.ajax({
        ...
        success:function (response) {
            ...
            var ctx = document.getElementById("d_goal_curve_chart");
            if (ctx) {
                ctx.height = 130;
                var myChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: ['Date', 'Actual', 'Target'],
                        type: 'line',
                        datasets: [{
                            data: [1, 18, 9],
                            label: 'Dataset',
...
$(document).ready(function () {
    ...
    {
        if (typeof($(".vision-dashboard")[0]) != "undefined")
        {
        ...
        else
        {
            google.charts.load('current', {'packages': ['corechart']});
            // Set a callback to run when the Google Visualization API is loaded.
            google.charts.setOnLoadCallback(visionLineDrawChart);
            google.charts.setOnLoadCallback(visionPieDrawChart);
            google.charts.setOnLoadCallback(visionBarDrawChart);
            function visionLineDrawChart() {
                var data = new google.visualization.DataTable();
                //data.addColumn('date', 'Hire Date');
    ...
    {
        if (typeof($(".strategy-dashboard")[0]) != "undefined")
        {
        ...
        else
        {
            google.charts.load('current', {'packages': ['corechart']});
            // Set a callback to run when the Google Visualization API is loaded.
            google.charts.setOnLoadCallback(strategyLineDrawChart);
            google.charts.setOnLoadCallback(strategyPieDrawChart);
            google.charts.setOnLoadCallback(strategyBarDrawChart);
            function strategyLineDrawChart() {
                var data = new google.visualization.DataTable();
    ...
    {
        if(typeof($(".kpi-dashboard")[0]) != "undefined")
        {
        ...
        else
        {
            google.charts.load('current', {'packages': ['corechart']});
            // Set a callback to run when the Google Visualization API is loaded.
            google.charts.setOnLoadCallback(kpiLineDrawChart);
            google.charts.setOnLoadCallback(kpiPieDrawChart);
            google.charts.setOnLoadCallback(kpiBarDrawChart);
            function kpiLineDrawChart() {
                var data = new google.visualization.DataTable();
                data.addColumn('string', 'Date');
                data.addColumn('number', 'Actual');
    ...
    {
        if(typeof($(".goal-dashboard")[0]) != "undefined")
        {
    ...
    {
        if(typeof($(".goalData-dashboard")[0]) != "undefined")
        {
