...
@section("page-header")
    <div class="ks-title">
        ...
        <div class="col-sm-3">
            <h3 class="text-dark"> Assign Goal Here</h3>
        </div>
...
@section("main-container")
    ...
    <table id="ks-datatable" class="table table-striped table-bordered" width="100%">
        ...
        <tbody>
        @forelse($allGoals as $allGoal)
            {{--reason for storing id with Goal is to use it in case of deleting--}}
            {{--and updating with ajax--}}
            <tr id="assign-Goal{{$allGoal->id}}">
                <td>{{$allGoal->name}}</td>
                <td>{{$allGoal->description}}</td>
                {{--count of the total no of users against one Goal--}}
                <td>{{$allGoal->users->count()}}</td>
                <td>
                    <div class='ks-controls dropdown'>
                        ...
                        <div class='dropdown-menu dropdown-menu-right' x-placement='bottom-end'
                            ...
                            <a class='dropdown-item assignUserLink' href='#' data-id='{{$allGoal->id}}'
                               data-toggle='modal' data-name="goal" data-target='#assignUserModal'>
                                <span class='la la-users ks-icon'></span>Assign</a>
                            <a class='dropdown-item updateAssignUserLink' href='#' data-id='{{$allGoal->id}}'
                               data-toggle='modal' data-name="goal" data-target='#updateAssignUserModal'>
            ...
            <div class="modal-body">
                <form id="assignUserForm">
                    ...
                    <div class="form-group">
                        <label for="Goal-name" class="col-form-label">@yield("page-title") Name:</label>
