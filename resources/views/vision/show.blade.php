...
@section("page-header")
    <div class="ks-title">
        <h3 class="text-primary">{{$visionData->name}} Details</h3>
    </div>
@endsection
            ...
            <div class="ks-wrapper">
               ...
               <span class="ks-name">
                   {{$visionData->name}}
                        ...
                        &nbsp;
                        <div class="">
                            <span class="ks-icon la la-map-marker ks-color-success">Start Date</span> {{$visionData->start_date}}
                            <span class="ks-icon la la-flag ks-color-success">End Date</span>{{$visionData->end_date}}
                        </div>
                    ...
                    <div class="ks-project">
                        Created At
                        <span class="ks-target ks-color-info">&nbsp;{{$visionData->created_at}}</span>
                        &nbsp;
                        {{--Created By--}}
                        {{--<span class="ks-target ks-color-success">{{\App\Models\User::find($visionData->user_id)->name}}</span>--}}
                    </div>
                ...
                <div class="card panel ks-attach-files-widget">
                    <h5 class="card-header">{{$visionData->strategies->count()}} Strategies agaisnt {{$visionData->name}} are as follows </h5>
                    <div class="card-block">
                        @foreach($visionData->strategies as $strategy)
                            <ul class="ks-uploaded-files">
                                <li>
                                    <span class="ks-icon la la-bar-chart ks-color-primary"></span>
                                    <div class="ks-body">
                                        <div class="ks-header">
                                            <span class="ks-filename">{{$strategy->name}}</span>
                                        </div>
                                ...
                                <table class="ks-table-details">
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            ...
                                            <div class="ks-body">
                                                {{$visionData->start_date->toFormattedDateString() . "    ,    " . $visionData->end_date->toFormattedDateString()}}
                                            </div>
                                        </td>
                                    </tr>
                                    </tbody>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="ks-body">
                                                <p class="ks-user">
                                                <ul>
                                                    @forelse($visionData->users as $user)
                                                        <li>
