...
@section("page-header")
    <div class="ks-title">
        ...
        <div class="col-sm-3">
            <h3 class="text-dark"> Assign Vision Here</h3>
        </div>
...
@section("main-container")
    ...
    <table id="ks-datatable" class="table table-striped table-bordered" width="100%">
        ...
        <tbody>
        @forelse($allVisions as $allVision)
            {{--reason for storing id with Vision is to use it in case of deleting--}}
            {{--and updating with ajax--}}
            <tr id="assign-Vision{{$allVision->id}}">
                <td>{{$allVision->name}}</td>
                <td>{{$allVision->description}}</td>
                {{--count of the total no of users against one Vision--}}
                <td>{{$allVision->users->count()}}</td>
                <td>
                    <div class='ks-controls dropdown'>
                        ...
                        <div class='dropdown-menu dropdown-menu-right' x-placement='bottom-end'
                            ...
                            <a class='dropdown-item assignUserLink' href='#' data-id='{{$allVision->id}}'
                               data-toggle='modal' data-name="vision" data-target='#assignUserModal'>
                                <span class='la la-users ks-icon'></span>Assign</a>
                            <a class='dropdown-item updateAssignUserLink' href='#' data-id='{{$allVision->id}}'
                               data-toggle='modal' data-name="vision" data-target='#updateAssignUserModal'>
            ...
            <div class="modal-body">
                <form id="assignUserForm">
                    ...
                    <div class="form-group">
                        <label for="Vision-name" class="col-form-label">@yield("page-title") Name:</label>
